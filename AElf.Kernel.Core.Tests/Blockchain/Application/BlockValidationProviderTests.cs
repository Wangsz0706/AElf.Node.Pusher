...
public sealed class BlockValidationProviderTests : AElfKernelWithChainTestBase
{
    ...
    {
        var validateResult = await _blockValidationProvider.ValidateBlockBeforeExecuteAsync(null);
    ...
    {
        var validateResult = await _blockValidationProvider.ValidateBeforeAttachAsync(null);
        validateResult.ShouldBeFalse();
    ...
    public void ExceptionTest()
    {
        ...
        Should.Throw<ValidateNextTimeBlockValidationException>(() =>
            throw new ValidateNextTimeBlockValidationException());
        Should.Throw<ValidateNextTimeBlockValidationException>(() =>
            throw new ValidateNextTimeBlockValidationException(message));
        Should.Throw<ValidateNextTimeBlockValidationException>(() =>
            throw new ValidateNextTimeBlockValidationException(message, exception));
        Should.Throw<ValidateNextTimeBlockValidationException>(() =>
            throw new ValidateNextTimeBlockValidationException(Hash.Empty));
    }
}