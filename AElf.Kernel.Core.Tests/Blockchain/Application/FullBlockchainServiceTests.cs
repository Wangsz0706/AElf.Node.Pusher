...
public class FullBlockchainServiceTests : AElfKernelWithChainTestBase
{
    ...
    {
        var result =
            await _fullBlockchainService.GetBlocksInBestChainBranchAsync(HashHelper.ComputeFrom("not exist"), 3);
        result.Count.ShouldBe(0);
    }
    ...
    {
        var transactions = new List<Transaction>();
        for (var i = 0; i < 3; i++) transactions.Add(_kernelTestHelper.GenerateTransaction());
