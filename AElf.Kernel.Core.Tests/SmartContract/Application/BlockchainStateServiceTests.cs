...
public sealed class BlockchainStateServiceTests : AElfKernelWithChainTestBase
{
    ...
    public async Task BlockState_Merge_GotException_Test()
    {
        ...
        await Should.ThrowAsync<InvalidOperationException>(() => _blockchainStateService.MergeBlockStateAsync(
    ...
    {
        for (var i = 0; i < 5; i++) await AddBlockStateSetAsync(_kernelTestHelper.BestBranchBlockList[i]);
